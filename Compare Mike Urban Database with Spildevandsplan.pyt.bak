...
class GetImperviousnessSPV(object):
    ...
    def execute(self, parameters, messages):
        ...
        MU_database = re.findall(r"(.+(?:\.mdb))", arcpy.Describe(CatchmentLayer).catalogPath)[0]
        SPV_feature = parameters[1].ValueAsText
        ...
        ms_Catchment = arcpy.CopyFeatures_management(CatchmentLayer, getAvailableFilename(arcpy.env.scratchGDB + "\ms_CatchmentImp", parent = MU_database))
