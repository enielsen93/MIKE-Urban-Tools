            ...
            with sqlite3.connect(
                ...
                with arcpy.da.SearchCursor(arcpy.Describe(pipe_layer).catalogPath, ["MUID", "Diameter", "MaterialID"],
                    ...
                    for row in cursor:
                        ...
                        arcpy.AddMessage("Upgraded pipe %s from %d to %d" % (row[0], oldDiameter, diameter))
        else:
...
class CopyDiameter(object):
    ...
    def updateParameters(self, parameters): #optional
        if parameters[0].altered:
            ...
            parameters[2].filter.list = [f.name for f in arcpy.Describe(reference_feature_layer).fields]
    ...
    def execute(self, parameters, messages):
        ...
        references = []
        fields = [field.name for field in arcpy.ListFields(reference_feature_layer)]
        ...
        fields = [field.name for field in arcpy.ListFields(target_feature_layer)]
