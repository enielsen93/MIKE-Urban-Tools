...
extension = "20231211_spill"

MU_model = None
MU_model = r"C:\Users\ELNN\OneDrive - Ramboll\Documents\Aarhus Vand\Vesterbro Torv\MIKE_URBAN\VBT_STATUS_011\VBT_STATUS_011.sqlite"
...
res1d_file = r"C:\Users\ELNN\OneDrive - Ramboll\Documents\Aarhus Vand\Vesterbro Torv\MIKE_URBAN\05_RESULTS\01_NETWORK\2023-12-11\VBT_219_mGA_CDS_20_138_SpillBaseDefault_LTS_extreme_statistics.res1d"
...
class Node:
    ...
    @property
    def spilling(self):
        return True if self.cover_type_no == 3 else False
...
with arcpy.da.InsertCursor(nodes_output_filepath, ["SHAPE@", "MUID", "Diameter", "Invert_lev", "Flood_vol"]) as cursor:
    with alive_bar(len(list(df.data.Nodes)), force_tty=True) as bar:
        for query_node in df.data.Nodes:
            ...
            try:
                if any(["WaterSpillDischargeIntegrated" in str(di.Quantity) for di in res1d.nodes[muid].DataItems]):
